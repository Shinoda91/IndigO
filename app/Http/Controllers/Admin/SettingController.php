...
class SettingController extends Controller
{
    ...
    public function store(AdminUpdateSettingsRequest $request)
    {
        ...
        if ($request->hasFile('banner')) {
            $image = new Upload($request->file('banner'), 'settings');
            $settings->banner = $image->getFullPath();
        }

        // IMAGE LOGO
        if ($request->hasFile('logo')) {
            $image = new Upload($request->file('logo'), 'settings');
            $settings->logo = $image->getFullPath();
        }
        // IMAGE LOGO
        if ($request->hasFile('favicon')) {
            $image = new Upload($request->file('favicon'), 'settings');
    ...
    {
        $file = new Upload($request->file('image'), 'settings');
